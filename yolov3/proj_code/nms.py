...
def non_maximum_suppression(net_output, confidence, num_classes, nms_threshold = 0.4):
    
    # net_output = net_output.numpy()
    confidence_mask = tf.expand_dims(tf.cast((net_output[:,:,4] > confidence), tf.float32), 2)
